...
export const stripePayment: StripePayment<string, StripePaymentResult> = async (tier, context) => {
	...
	let priceId;
	if (tier === TierIds.SINGLEUSER) {
		priceId = process.env.SINGLEUSER_SUBSCRIPTION_PRICE_ID!;
	} else if (tier === TierIds.SMALLBUSINESS) {
		priceId = process.env.SMALLBUSINESS_SUBSCRIPTION_PRICE_ID!;
	} else if (tier === TierIds.LARGEBUSINESS) {
		priceId = process.env.LARGEBUSINESS_SUBSCRIPTION_PRICE_ID!;
	} else {
