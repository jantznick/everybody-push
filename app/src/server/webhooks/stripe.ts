...
export const stripeWebhook: StripeWebhook = async (request, response, context) => {
	...
	try {
		if (event.type === 'checkout.session.completed') {
			...
			if (line_items?.data[0]?.price?.id === process.env.SINGLEUSER_SUBSCRIPTION_PRICE_ID) {
				console.log('Single user subscription purchased ');
				await context.entities.User.updateMany({
					...
					data: {
						hasPaid: true,
						datePaid: new Date(),
						subscriptionTier: TierIds.SINGLEUSER,
					},
				});
			} else if (line_items?.data[0]?.price?.id === process.env.SMALLBUSINESS_SUBSCRIPTION_PRICE_ID) {
				console.log('Small business subscription purchased ');
				await context.entities.User.updateMany({
					where: {
						stripeId: userStripeId,
					},
					data: {
						hasPaid: true,
						datePaid: new Date(),
						subscriptionTier: TierIds.SMALLBUSINESS,
					},
				});
			} else if (line_items?.data[0]?.price?.id === process.env.LARGEBUSINESS_SUBSCRIPTION_PRICE_ID) {
				console.log('Large business subscription purchased ');
				await context.entities.User.updateMany({
					where: {
						stripeId: userStripeId,
					},
					data: {
						hasPaid: true,
						datePaid: new Date(),
						subscriptionTier: TierIds.LARGEBUSINESS,
					},
				});
			}
